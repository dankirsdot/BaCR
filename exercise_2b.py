...
for e in range(episodes):
    ...
    for net in range(networks):
        ...
        for _ in range(steps):
            if net == 0: # render only the object driven by the first neural networks
                env.render()
                time.sleep(0.01)
    ...
    best = reward_sum.argsort()[-5:][::-1] # find 5 best neural networks
    worst = reward_sum.argsort()[:5][::-1] # find 5 worst neural networks
    print("Reward during the episode {}".format(reward_sum))
    print("5 best neural networks {}".format(best))
